...
- `proposals`: records where `status = "Proposta"`
- `sales`: records where `status = "Venda"`

### Scenario: Funnel counts computed correctly
- **WHEN** a group contains 10 records, 3 with `status = "Proposta"`, 1 with `status = "Venda"`
- **THEN** response shows `{ leads: 10, proposals: 3, sales: 1 }`

### Scenario: No proposals or sales
- **WHEN** all records in a group have null status

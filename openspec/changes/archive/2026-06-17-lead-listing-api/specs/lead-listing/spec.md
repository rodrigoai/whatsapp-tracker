...
The endpoint SHALL accept an optional `status` query parameter (repeatable). Valid values are `Not Qualified`, `Proposta`, and `Venda`. `Not Qualified` MUST match leads where the DB `status` field is `NULL`. Multiple values are combined with OR. An unrecognized status value SHALL return 400.

#### Scenario: Filter by single status
- **WHEN** a request includes `status=Venda`
- **THEN** only leads with `status = 'Venda'` in the database are returned

#### Scenario: Filter by Not Qualified maps to null
- **WHEN** a request includes `status=Not%20Qualified`
- **THEN** only leads where `status IS NULL` in the database are returned

#### Scenario: Multiple status values
- **WHEN** a request includes `status=Proposta&status=Venda`
- **THEN** leads where `status = 'Proposta'` OR `status = 'Venda'` are returned
